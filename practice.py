...
def repeat_string(str1, int1):
    """Takes string and integer and prints it many times"""
    print str1 * int1
...
"""Takes integer and prints if higher than 0, lower than 0, or zero"""
def print_sign(int1):
    if int1 > 0: 
...
def is_divisible_by_three(number):
    """takes integer and returns bool true or false if number is divisible by three"""
    if number % 3 == 0:
        return True
    else: 
        return False
...
def engineer_job_title(name, title):
...
#    greeting.
def letter_greeting(title, name, sender_name):
    print "Dear %s.upper %s.upper, I think you are amazing! Sincerely, %s.upper" % (title, name, sender_name)
